...
class Warehouse_model extends App_Model {
	...
	public function get_stock_export_pdf_html($goods_delivery_id) {
		...
		if (is_numeric($goods_delivery->customer_code) && $goods_delivery->customer_code != 0) {
			$customer = $this->db->get_where(db_prefix() . 'clients', ['userid' => $goods_delivery->customer_code])->row();
			if ($customer) {
				$customer_no      = $customer->userid;
				$customer_name    = $customer->company;
				$customer_address = trim(implode(', ', array_filter([$customer->billing_street, $customer->billing_city, $customer->billing_state])));
				if ($customer_address == '') {
					$customer_address = trim(implode(', ', array_filter([$customer->address, $customer->city, $customer->state])));
				}
			}
		}
		...
		if (isset($goods_delivery->invoice_id) && is_numeric($goods_delivery->invoice_id) && $goods_delivery->invoice_id != 0) {
			$invoice = $this->db->get_where(db_prefix() . 'invoices', ['id' => $goods_delivery->invoice_id])->row();
			if ($invoice) {
				$invoice_number = format_invoice_number($goods_delivery->invoice_id);
				$invoice_date   = $invoice->date;
			}

			$dc_serials = $this->db->select('item_id, serial_number')->from(db_prefix() . 'itemserials')->where('si_id', $goods_delivery->invoice_id)->order_by('serial_number', 'ASC')->get()->result_array();
			foreach ($dc_serials as $dc_serial) {
				$serials_by_item[$dc_serial['item_id']][] = $dc_serial['serial_number'];
			}
		}
		if ($invoice_number == '') {
			$invoice_number = (isset($goods_delivery->goods_delivery_code) ? $goods_delivery->goods_delivery_code : '');
		}
		if ($invoice_date == '') {
			$invoice_date = $goods_delivery->date_add;
		}
		...
		$html .= '</tr>';
		$html .= '<tr>';
		$html .= '<td width="164"><b>Customer Name</b></td><td width="581">' . htmlspecialchars($customer_name) . '</td>';
		$html .= '</tr>';
		$html .= '<tr>';
		$html .= '<td width="164"><b>Customer Address</b></td><td width="581">' . htmlspecialchars($customer_address) . '</td>';
		$html .= '</tr>';
		$html .= '<tr>';
		$html .= '<td width="164"><b>Invoice Date</b></td><td width="581">' . _d($invoice_date) . '</td>';
		$html .= '</tr>';
		...
		if (count($goods_delivery_detail) > 0) {
			foreach ($goods_delivery_detail as $delivery_value) {
				$commodity_code = '';
				$commodity_name = (isset($delivery_value['commodity_name']) ? $delivery_value['commodity_name'] : '');
				$serial_no      = '';
				$quantities     = (isset($delivery_value['quantities']) ? $delivery_value['quantities'] : 0);

				$item_info = get_commodity_name($delivery_value['commodity_code']);
				if ($item_info != null) {
					$commodity_code = $item_info->commodity_code;
					$commodity_name = $item_info->description;
				}
				...
				if (isset($serials_by_item[$delivery_value['commodity_code']])) {
					$serial_no = implode(', ', $serials_by_item[$delivery_value['commodity_code']]);
				} elseif ($commodity_code != '' && isset($serials_by_item[$commodity_code])) {
					$serial_no = implode(', ', $serials_by_item[$commodity_code]);
				}
				if ($serial_no == '') {
					$serial_no = (isset($delivery_value['lot_number']) ? $delivery_value['lot_number'] : '');
				}
		...
		$html .= '<br /><br />';
		$html .= '<table width="745" cellspacing="0" style="font-size:18px;">';
		$html .= '<tr><td width="745"><b>Note:</b> Please Check the Serial No. and quantiles at the time of Physically receiving the stocks.</td></tr>';
		$html .= '</table>';

		$html .= '<br /><br /><br /><br />';
		$html .= '<table width="745" cellspacing="0" style="font-size:18px;">';
		$html .= '<tr><td width="545"></td><td width="200" align="right"><b>Customer Signature and Stamp</b></td></tr>';
		$html .= '</table>';
