...
// require_once APPPATH.'third_party/pdfparser/src/Smalot/PdfParser/Parser.php';
...
public function delete_hrm_staff_attachment($attachment_id)
    ...
    public function ai_resume_analyzer(){
        if($this->input->post()){
            ...
            if($this->upload->do_upload('resume')){
                ...
                $post['resume'] = $uploadData['file_name'];
                $post['file_type'] = $uploadData['file_type'];
            }else{
                $post['resume'] = null;
            }           
            // print_r($post);exit;
            $inserted = $this->hrm_model->insert_resume($post);
            if($inserted){
                $this->session->set_flashdata('success','Resume added successfully.');
                redirect('hrm/analyze_resume/'.$inserted);
            }else{
                $this->session->set_flashdata('error','Failed to add resume. Please try again');
                redirect('hrm/ai_resume_analyzer');
            }
        }
        $data['title'] = _l('ai_resume_analyzer');
        $this->load->view('hrm/ai_resume_analyzer',$data);
    }
    public function analyze_resume($id = ''){
        if ($id == '') {
            redirect('hrm/ai_resume_analyzer');
        }

        $resume = $this->hrm_model->get_resume_by_id($id);
        if (!$resume || empty($resume->resume)) {
            set_alert('danger', 'Resume file not found.');
            redirect('hrm/ai_resume_analyzer');
        }

        $filePath = FCPATH . 'uploads/resumes/' . $resume->resume;

        if (!file_exists($filePath)) {
            set_alert('danger', 'Resume file missing from server.');
            redirect('hrm/ai_resume_analyzer');
        }
        ...
        try {
            ...
            set_alert('danger', 'Failed to read resume file.');
            redirect('hrm/ai_resume_analyzer');
        }

        if (empty(trim($resumeText))) {
            set_alert('danger', 'Resume text could not be extracted.');
            redirect('hrm/ai_resume_analyzer');
        }
    ...
    ";
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_HTTPHEADER => [
                ...
                'Authorization: Bearer BEARER_API_KEY_HERE' // Replace with your actual API key
            ]
        ]);
        ...
        if (curl_errno($ch)) {
            curl_close($ch);
            set_alert('danger', 'AI request failed.');
            redirect('hrm/ai_resume_analyzer');
        }
        ...
        if (!isset($result['choices'][0]['message']['content'])) {
            set_alert('danger', 'Invalid AI response.');
            redirect('hrm/ai_resume_analyzer');
        }
        ...
        STEP 4: SHOW RESULT
        ================================ */        
        $data['analysis'] = nl2br($analysis);
        $this->db->where('id', $id);
        $this->db->update('tblai_resume_analysis', ['response' => $analysis]);
        print_r($data['analysis']);exit;
        
        redirect('hrm/ai_resume_analyzer');
    }
    // public function test(){
    //     $parser = new Parser();
    //     $pdf = $parser->parseFile(FCPATH.'uploads/resumes/test.pdf');
    //     $text = $pdf->getText();
