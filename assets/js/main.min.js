...
function add_item_to_table(data, itemid, merge_invoice, bill_expense) {
  ...
  get_taxes_dropdown_template(tax_name, data.taxname).done(function (
    ...
    table_row +=
      ...
      item_key +
      '][item_code]" value="' + (data.item_code || '') + '">';
    ...
    table_row +=
      ...
      "</td>";
    table_row += '<td class="taxrate"><input type="number" class="form-control tax-input" min="0" step="0.01" name="newitems[' + item_key + '][total_tax]" value="0"></td>';

    table_row +=
      '<td class="amount_after_tax" align="right">' +
      format_money(amount, true) +
      "</td>";
    table_row +=
      '<td align="center"><select class="selectpicker serials-select" data-width="100%" multiple data-none-selected-text="-" data-item-code="' + (data.item_code || '') + '" name="newitems[' + item_key + '][serials][]"></select></td>';
    ...
    // Load serials directly
    if(data.item_code){
      var $serSel = $("table.items tbody tr.item:last .serials-select");
      if($serSel.length){
        $.get(admin_url + "purchase/get_available_serials/" + data.item_code).done(function(resp) {
